...
/**
 * 网络加载ListView demo
 * 此demo演示的是自定义参数的生成使用过程
 * Created By: Seal.Wu
 * Date: 2015/9/30
 * Time: 10:05
 */
public class NetListViewWithParamMakerActivity extends Activity {
        ...
        listView.setParamMaker(new NLVCommonInterface.ParamMaker() {
            ...
            public <ParamObj> RequestParams<ParamObj> makeParam(RequestParams<ParamObj> preParam, boolean isInit) {
                if (isInit) {
                    ...
                    params.putParams("token", "Zrmp6OJN8JilVNd66DSRntEQAzPtNXNdQzGUK8FDhavb9Lv%2BGSEmpqVvPXLgk0S00F1isuQY5R4%3D");
                    params.putParams("userId", "600000032");
                    params.putParams("pageNo1", "0");
                    return params;
                } else {
                    if (preParam != null) {
                        final int pageNo = Integer.parseInt(preParam.getParam("pageNo1"));
                        preParam.setParams("pageNo1", (pageNo + 1) + "");
