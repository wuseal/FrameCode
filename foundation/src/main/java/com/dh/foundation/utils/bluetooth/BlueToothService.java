...
class BlueToothService implements IBlueToothService {
    ...
    private BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (StringUtils.equals(BluetoothAdapter.ACTION_DISCOVERY_STARTED, action)) {
                DLoggerUtils.i("Start search BlueToothDevices");
            } else if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                BluetoothDevice bluetoothDevice = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
                DLoggerUtils.i("Search---founded BlueToothDevice: " + bluetoothDevice.getName() + "   " + bluetoothDevice.getAddress());
                for (BluetoothDevice bluetoothDevice1 : boundBluetoothDevice) {
                    if (StringUtils.equals(bluetoothDevice1.getAddress(), bluetoothDevice.getAddress())) {
                        return;
                    }
                }
                ...
                deviceReceiver.unBoundDeviceReceive(unBoundBluetoothDevice);
                deviceReceiver.onFinishedReceive();
                DLoggerUtils.i("Finished Search BlueToothDevices");
            }
        }
    };
}
